...
func resourceArmKeyVaultAccessPolicyCreateOrDelete(d *schema.ResourceData, meta interface{}, action keyvault.AccessPolicyUpdateKind) error {
	...
	if err != nil {
		...
		// a bad ID was given.
		if response.ResponseWasNotFound(keyVault.Response) && !d.IsNewResource() {
	...
	azureStackLockByName(vaultName, keyVaultResourceName)
	defer azureStackUnlockByName(vaultName, keyVaultResourceName)
	...
	stateConf := &resource.StateChangeConf{
		...
		Target:                    []string{"found"},
		Refresh:                   accessPolicyRefreshFunc(ctx, &meta.(*ArmClient).keyVaultClient, resourceGroup, vaultName, objectId, applicationIdRaw),
		Delay:                     5 * time.Second,
