...
func resourceArmSubnetNetworkSecurityGroupAssociationCreate(d *schema.ResourceData, meta interface{}) error {
	...
	azureStackLockByName(parsedNetworkSecurityGroupId.Name, networkSecurityGroupResourceName)
	defer azureStackUnlockByName(parsedNetworkSecurityGroupId.Name, networkSecurityGroupResourceName)

	subnetName := parsedSubnetId.Path["subnets"]
	virtualNetworkName := parsedSubnetId.Path["virtualNetworks"]
	resourceGroup := parsedSubnetId.ResourceGroup

	azureStackLockByName(subnetName, subnetResourceName)
